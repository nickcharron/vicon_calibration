...
public:
   ...
   * vicon_calibratoin/config/CeresParamsDefault.json
   *
   */
  CeresParams(const std::string& config_path,
    ...
    nlohmann::json J;
    if (!utils::ReadJson(config_path, J)) {
      LOG_ERROR("Using default ceres params.");
    }
    ...
    try {
      ...
      LOG_ERROR(
          ...
          "params are missing or invalid. Using default parameters. Reason: %s",
          e.what());
    }
  }
