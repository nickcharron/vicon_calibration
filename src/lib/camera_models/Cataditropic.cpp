...
std::shared_ptr<CameraModel> Cataditropic::Clone() {
  ...
  clone->SetIntrinsics(GetIntrinsics());
  clone->SetImageDims(GetHeight(), GetWidth());
  clone->SetFrameID(GetFrameID());
  clone->SetCalibrationDate(GetCalibrationDate());
  return clone;
}
...
bool Cataditropic::ProjectPoint(const Eigen::Vector3d& in_point,
  ...
  if (!InProjectionDomain(in_point)) { return false; }
  ...
  if (J) { LOG_WARN("Jacobian not yet implemented."); }
  return true;
}
